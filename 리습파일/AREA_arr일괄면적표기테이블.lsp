...
(defun c:arr ( / doc space ss n sum obj sp ep cplst area sum cdn mp v sa xp yp pt
	...
	(foreach x arlst
		...
		;(setq txtobj (vla-addtext space (strcat (rtos n) " : " (cadr x)) pt $tsz)) ;no number (strcat (rtos n) " : " (cadr x))  ⇒  (cadr x); 원본
		(setq txtobj (vla-addtext space (strcat (rtos n 2 0) ") 면적(M2) : " (cadr x)) pt $tsz)) ;no number (strcat (rtos n) " : " (cadr x))  ⇒  (cadr x)
